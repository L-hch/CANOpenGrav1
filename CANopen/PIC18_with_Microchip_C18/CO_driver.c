...
#elif CO_OSCILATOR_FREQ == 64
  {{64,  1, 8, 8, 8}, //CAN=10kbps   // not possible
   {50,  1, 5, 8, 2}, //CAN=20kbps   // not possible
   {40,  1, 5, 8, 2}, //CAN=50kbps  
   {16,   1, 5, 8, 2}, //CAN=125kbps
   {8,   1, 5, 8, 2}, //CAN=250kbps
   {4,   1, 5, 8, 2}, //CAN=500kbps
   {4,   1, 3, 4, 2}, //CAN=800kbps
   {4,   1, 2, 3, 2}};//CAN=1000kbps
...
void CO_Read_NodeId_BitRate(void){
   ...
   // ReadDipSwitches is on user.c file
   extern unsigned char ReadDipSwitches(void);
   ODE_CANnodeID = ReadDipSwitches();
#endif
...
/* Setup CAN bus */
   ...
   TXB2CONbits.TXREQ = 0;
#if  __18F26K80
   PIE5 = 0;            //CAN interrupts disable
   PIR5 = 0;            //CAN interrupts flags clear
   IPR5 = 0;            //CAN interrupts low priority set
#else
...
#if  __18F26K80
   PIE5 = 0x07;         //enable two receive interrupts and TX0 interrupt
   IPR5 = 0x03;         //receive interrupts are high priority
#else
...
#pragma    MSG_PIC18F26K80_in_USE

 
   if(PIR5bits.RXB0IF){ //RXB0 occurred
      PIR5bits.RXB0IF = 0;
#else
#pragma    MSG_PIC18F258_in_USE
